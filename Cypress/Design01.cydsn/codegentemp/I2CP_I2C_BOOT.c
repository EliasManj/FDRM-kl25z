...
static uint8 I2CP_slReadBuf[I2CP_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 I2CP_slWriteBuf[I2CP_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 I2CP_applyBuffer;
...
static void I2CP_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: I2CP_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  I2CP_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void I2CP_I2CCyBtldrCommStart(void)
{
    I2CP_I2CSlaveInitWriteBuf(I2CP_slWriteBuf, I2CP_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    I2CP_I2CSlaveInitReadBuf (I2CP_slReadBuf, 0u);

    I2CP_SetCustomInterruptHandler(&I2CP_I2CResposeInsert);
    I2CP_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: I2CP_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2CP_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  I2CP_slRdBufSize - used to store slave read buffer size.
*  I2CP_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  I2CP_slWrBufIndex - used to store current index within slave
*  write buffer.
*  I2CP_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
...
/*******************************************************************************
* Function Name: I2CP_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  I2CP_slStatus  - used to store current status of I2C slave.
*  I2CP_slWriteBuf - used to store received command.
*  I2CP_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus I2CP_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (I2CP_I2C_SSTAT_WR_CMPLT & I2CP_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) I2CP_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) I2CP_slWriteBuf,
                                              I2CP_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(I2CP_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: I2CP_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  I2CP_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus I2CP_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) I2CP_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        I2CP_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: I2CP_I2CResposeInsert
****************************************************************************//**
*
...
static void I2CP_I2CResposeInsert(void)
{
    if (I2CP_CHECK_INTR_SLAVE_MASKED(I2CP_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (I2CP_CHECK_I2C_STATUS(I2CP_I2C_STATUS_S_READ))
        {
            ...
            if (0u != I2CP_applyBuffer)
            {
                I2CP_slRdBufSize  = I2CP_applyBuffer;
                I2CP_slRdBufIndex = 0u;
                I2CP_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (I2CP_slRdBufIndex != I2CP_slRdBufSize)
            {
                I2CP_slRdBufIndex = I2CP_slRdBufSize;
            }
        }
    }
}
